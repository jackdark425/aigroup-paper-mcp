...
function shouldStartMCPServer(): boolean {
  // 显式要求MCP服务器模式
  if (process.env.MCP_SERVER_MODE === 'true' || process.argv.includes('--mcp-server')) {
    return true;
  }
  
  // 检查是否有CLI子命令
  const knownCommands = ['search', 'advanced-search', 'fetch', 'latest', 'categories', 'trends'];
  const hasCommand = process.argv.slice(2).some(arg =>
    knownCommands.includes(arg) || arg === '--help' || arg === '-h' || arg === '--version' || arg === '-V'
  );
...
async function startMCPServer(): Promise<void> {
  try {
    const { MCPServer } = await import('./services/mcp-server.js');
